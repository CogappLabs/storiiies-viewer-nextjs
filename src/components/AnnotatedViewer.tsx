...
interface Props {
	...
	onAnnotationCreate?: (data: AnnotationData) => void;
	onAnnotationUpdate?: (
		id: string,
...
const AnnotatedViewer = ({
			...
			import("@recogito/annotorious-openseadragon").then((Annotorious) => {
				...
				const updateCrosshairs = (shapeGroup: Element) => {
					...
					if (!group) {
						...
						hLine.setAttribute("stroke", "#3b82f6");
						...
						vLine.setAttribute("stroke", "#3b82f6");
	...
	useEffect(() => {
		...
		// With disableEditor: true, we get createSelection instead of createAnnotation
		const handleCreate = (w3cAnnotation: W3CAnnotation) => {
			const coords = parseFragment(w3cAnnotation.target.selector.value);
			const viewer = viewerRef.current;

			if (coords && onAnnotationCreate && viewer) {
				...
				// Cancel the selection - we'll add it back after saving to DB
				anno.cancelSelected();
			}
		};
	...
	// Handle drawing enabled state
	useEffect(() => {
		annoRef.current?.setDrawingEnabled(drawingEnabled);
	}, [drawingEnabled]);

	// Handle selected annotation
	const panToAnnotation = useCallback((annotationId: string) => {
		annoRef.current?.fitBounds(annotationId, false);
	}, []);

	// Expose panToAnnotation via ref or callback
	useEffect(() => {
	...
	return (
		<div className="relative h-full w-full">
			...
			{drawingEnabled && (
				<div className="absolute top-4 right-4 bg-cogapp-charcoal text-cogapp-cream px-3 pt-2 pb-1.5 text-base font-medium border border-cogapp-cream z-10">
